"""Support for reading HeaterMeter data.

See https://github.com/CapnBry/HeaterMeter/wiki/Accessing-Raw-Data-Remotely
    ...
    def handle_setpoint_api(call):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                _LOGGER.debug(
                    "HeaterMeter handle_setpoint_api wrong API version, reverting to old setpoint api"
                )
                handle_setpoint(temp)
            elif r.status_code == 403:
                _LOGGER.debug(
                    "HeaterMeter handle_setpoint_api User has explicitly disabled external API"
                )
            else:
                _LOGGER.debug(
                    "HeaterMeter handle_setpoint_api unknown http reply %s", temp
                )

        except requests.exceptions.RequestException as e:  # This is the correct syntax
            _LOGGER.error(
                "HeaterMeter handle_setpoint Post Connection error %s", r.status_code
            )
    ...
    def handle_setalarms_api(call):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                _LOGGER.debug(
                    "HeaterMeter handle_setalarms_api wrong API version, reverting to old api"
                )
                handle_setpoint(alrm)
            elif r.status_code == 403:
                _LOGGER.debug(
                    "HeaterMeter handle_setalarms_api User has explicitly disabled external API"
                )
            else:
                _LOGGER.debug(
                    "HeaterMeter handle_setalarms_api unknown http reply %s", alrm
                )

        except requests.exceptions.RequestException:
            _LOGGER.error(
                "HeaterMeter handle_setalarms Post Connection error %s", r.status_code
            )
